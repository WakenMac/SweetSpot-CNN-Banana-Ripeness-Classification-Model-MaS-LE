...
class ResNet50Transfer(nn.Module):
    def __init__(self, num_classes=4):
        ...
        # Selects all layers except for the 
        self.features = nn.Sequential(
            *list(resnet50_model.children())[:-1]
        )

        # ResNet50 outputs a total of 2048 feature maps 
        INPUT_FEATURES = 2048

        # ANN classifier block
        self.classifier = nn.Sequential(
            nn.Flatten(),               # flatten after GAP
            nn.Linear(INPUT_FEATURES, 128),        # ResNet50 outputs 2048 beefy feature maps
            nn.ReLU(),
            nn.Dropout(0.5),
...
class EarlyStopper:
    ...
    def check_stop(self, validation_loss, learning_rate, batch_size):
        if validation_loss < self.best_validation_loss - self.min_delta:
            ...
            torch.save(model.state_dict(), f'Saved Models\\resnet_50_model_{batch_size}_{learning_rate}.pth')
...
for batch_size in batch_size_list:
    ...
    for i in range(len(learning_rates)):
        ...
        for epoch in range(start, num_epochs + 1):
            ...
            # Creating the template for training_details11.csv
...
torch.save(model, 'Saved Models\\New Models\\ResNet50.pt')
